...
import type { DayName, ICateringPackage } from "../schemas/package.schema";
      ...
      <section className="rounded-md border border-border/70 bg-muted/15 p-4 md:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{i18n.details.package}</p>
          ...
          {item.days.map((day) => (
            <article key={day.day} className="rounded-md border border-border/70 bg-card p-4">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 pb-3">
                <p className="text-sm font-semibold text-foreground">{i18n.dayLabels[day.day as DayName] ?? day.day}</p>
                <span className="rounded-full bg-muted px-2 py-1 text-[11px] font-medium text-muted-foreground">
                  {day.variants.length} {day.variants.length === 1 ? i18n.form.variantLabel : i18n.form.variantsLabel}
                </span>
              </div>
