...
import { ChefHat, ChevronDown, Clock, Globe, LaptopMinimal, Minus, Moon, Plus, Sun } from "lucide-react";
...
const portalPackages: PortalPackage[] = [
  ...
  },
  {
    ...
    mealsByDay: [
      ...
      { day: "Monday", items: "Beef Steak (Medium Rare), Mashed Potatoes, Glazed Carrots" },
      { day: "Tuesday", items: "Lamb Chops, Mint Chutney, Jeera Rice" },
      { day: "Wednesday", items: "Prawn Malai Curry, Coconut Rice, Cucumber Salad" },
      { day: "Thursday", items: "Chicken Tikka Masala, Garlic Naan, Mixed Grill" },
      { day: "Friday", items: "Grilled Sea Bass, Lemon Butter Sauce, Steamed Veggies" },
    ],
  },
];
...
export function ClientPortalPage({ tenant }: ClientPortalPageProps) {
  const [selected, setSelected] = useState<Record<string, number>>({ standard: 1 });
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_hsl(var(--landing-alt-bg))_0%,_hsl(var(--background))_42%,_hsl(var(--background))_100%)] text-foreground">
      ...
      <main className="mx-auto w-full max-w-[1220px] px-4 py-8 sm:px-6 lg:px-8">
        <section className="mb-10 max-w-2xl">
          ...
          <h1 className="mt-6 text-5xl font-bold tracking-tight sm:text-6xl">Today&apos;s Menu</h1>
          <p className="mt-4 text-base leading-relaxed text-slate-600 dark:text-slate-400 sm:text-lg">
            Curated daily selections for your team. Select your preferred tier and adjust quantities below to build your
                ...
                >
                  <CardHeader className="flex flex-col gap-4 border-b border-border/70 bg-muted/20 p-5 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/70 bg-background/80 px-3 py-2">
                      ...
                      <span className="text-xs uppercase tracking-[0.06em] text-muted-foreground">/ day</span>
                      <QuantityControl
                        value={qty}
                        onChange={(next) => {
                  ...
                  <CardContent className="space-y-2 p-5 mt-2">
                    <button
                      ...
                      onClick={() => {
                        setExpandedMenus((prev) => ({ ...prev, [item.id]: !mealsExpanded }));
                      }}
                    >
                      ...
                      <div id={`menu-${item.id}`} className="space-y-2">
                        {item.mealsByDay.map((menu) => (
                          ...
                          >
                            ...
                            <p className="text-sm font-medium text-foreground/90">{menu.items}</p>
                          </div>
                        ))}
                      </div>
                    ) : null}
          ...
          <aside className="h-fit space-y-4 lg:sticky lg:top-8">
            <Card className="border-border/70 bg-card/95 shadow-sm">
              <CardHeader className="p-5 pb-3">
                ...
                <p className="text-sm text-muted-foreground">Drafting your experience</p>
              </CardHeader>
                ...
                {orderRows.length === 0 ? (
                  ...
                  orderRows.map((row) => (
                    ...
                    >
                      ...
                      <p className="font-semibold text-foreground">{currency.format(row.subtotal)}</p>
                    </div>
                  ))
                )}
                <div className="pt-3">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xl font-bold tracking-tight text-primary">{currency.format(total)}</p>
                  </div>
                </div>
