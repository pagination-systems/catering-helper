    ...
    <div className="space-y-5">
      <section className="rounded-md border border-border/70 bg-muted/15 p-4 md:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-start gap-2 sm:items-end">
            ...
            <p className="text-sm font-semibold text-foreground">{bdt.format(item.total)}</p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 border-t border-border/60 pt-4 text-xs sm:grid-cols-3">
          <div>
            ...
            <p className="mt-1 font-medium text-foreground">{dateTimeFormatter.format(item.createdAt)}</p>
          </div>
          <div>
            <p className="uppercase tracking-wide text-muted-foreground">Last Updated</p>
            <p className="mt-1 font-medium text-foreground">{dateTimeFormatter.format(item.updatedAt)}</p>
          </div>
          <div>
            <p className="uppercase tracking-wide text-muted-foreground">Delivery</p>
            <p className="mt-1 font-medium text-foreground">
              {item.deliveryDay}, {dateTimeFormatter.format(item.deliveryDate)}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div className="rounded-md border border-border/70 bg-card p-3.5">
          ...
          <p className="mt-2 text-base font-semibold text-foreground">{bdt.format(item.total)}</p>
        </div>
      </section>
          ...
          {item.items.map((orderItem) => (
            <article key={orderItem.id} className="rounded-md border border-border/70 bg-card p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <p className="text-sm font-semibold text-foreground">{bdt.format(orderItem.subtotal)}</p>
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t pt-3 text-xs text-muted-foreground">
                <p>
                  {orderItem.quantity} x {bdt.format(orderItem.pricePerMeal)}
                </p>
                <p>{dateTimeFormatter.format(orderItem.deliveryDate)}</p>
                <p className="font-medium text-foreground">{bdt.format(orderItem.subtotal)}</p>
              </div>
            </article>
          ))}
        </div>
