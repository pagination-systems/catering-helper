...
export const PackageDetails = ({ item }: PackageDetailsProps) => {
  ...
  const metrics = [
    {
      label: "Price / Meal",
      value: bdt.format(item.pricePerMeal),
      ...
      <section className="rounded-md border border-border/70 bg-muted/15 p-4 md:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-start gap-2 sm:items-end">
            ...
            <p className="text-sm font-semibold text-foreground">{bdt.format(item.pricePerMeal)}</p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 border-t border-border/60 pt-4 text-xs sm:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 font-medium text-foreground">{dateTimeFormatter.format(new Date(item.createdAt))}</p>
          </div>
          <div>
            <p className="uppercase tracking-wide text-muted-foreground">Last Updated</p>
            <p className="mt-1 font-medium text-foreground">{dateTimeFormatter.format(new Date(item.updatedAt))}</p>
          </div>
        </div>
      </section>
