...
export const CustomerLedgerTable = ({ data, handlePaginate }: CustomerLedgerTableProps) => {
  ...
  const columns: DataTableColumn<ICustomerLedger>[] = [
    ...
    },
    {
      ...
      header: "Total Amount",
      cell: (item) => bdt.format(item.totalAmount),
    },
    {
      accessorKey: "totalPaidAmount",
      header: "Total Paid",
      cell: (item) => <span className="font-medium text-emerald-700">{bdt.format(item.totalPaidAmount)}</span>,
    },
    {
