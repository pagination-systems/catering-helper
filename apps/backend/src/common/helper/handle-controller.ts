...
interface AuthenticatedRequestObject extends Omit<RequestObject, "session"> {
  session: ISession;
}
...
export type AuthenticatedControllerParams = {
  req: AuthenticatedRequestObject;
};

const buildRequestObject = (req: Request): RequestObject => {
  return {
    body: req.body,
    query: req.query,
    params: req.params,
...
export function handleController(controller: ({ req }: ControllerParams) => Promise<ResponseObject>) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const requestObject = buildRequestObject(req);
...
export function handleAuthenticatedController(
  controller: ({ req }: AuthenticatedControllerParams) => Promise<ResponseObject>,
) {
  return (req: Request, res: Response, next: NextFunction) => {
    if (!req.session) {
    ...
    const requestObject: AuthenticatedRequestObject = {
